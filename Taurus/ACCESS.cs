...
namespace Taurus
{
    public class ACCESS
    {
        ...
        {
            bool ret = File.Exists(SystemInfo.AccessDB);//查询文件是否存在
            if (!ret)
            {
                System.Resources.ResourceManager rm = null;
                try
                {
                    rm = new System.Resources.ResourceManager("Taurus.Properties.Resources", Assembly.GetExecutingAssembly());//创建一个ResourceWriter的对象用于写资源文件
                    byte[] byt = (byte[])rm.GetObject("AccessMDB");
                    FileStream fs = new FileStream(SystemInfo.AccessDB, FileMode.Create);
        ...
        public bool CompactJetDatabase(string fileName)
        {
            if (!File.Exists(fileName)) return false;
            string tempFile = Path.Combine(Path.GetDirectoryName(fileName), Guid.NewGuid().ToString("N") + ".mdb");
            const string ConnStr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=";
            try
            {
                object objJRO = Activator.CreateInstance(Type.GetTypeFromProgID("JRO.JetEngine"));
                object[] param = new object[] { ConnStr + fileName, ConnStr + tempFile };
        ...
        public bool RestoreDatabase(string SrcFile, string DestFile)
        {
            bool ret = false;
            string ConnStr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + SrcFile;
            ...
            try
            {
                ...
                ret = CompactJetDatabase(DestFile);
            }
            catch (Exception E)
            {
                Pub.ShowErrorMsg(E);
            }
            if (!SystemInfo.db.IsOpen) SystemInfo.db.Open();
            return ret;
        }

        private string GetMaxIDSQL(string FieldName, string TableName)
        {
            string ret = "SELECT IIF(ISNULL(MIN(CardID)),(SELECT IIF(ISNULL(MAX(" + FieldName + ")),0,MAX(" +
              FieldName + "))+1 FROM " + TableName + " WHERE ISNUMERIC(" + FieldName + ")),MIN(CardID)) AS CardIDX " +
              "FROM (SELECT(SELECT COUNT(1)+1 FROM " + TableName + " a WHERE ISNUMERIC(a." + FieldName +
              ") AND a." + FieldName + ">b." + FieldName + ") AS CardID FROM " + TableName +
              " b) a WHERE NOT EXISTS(SELECT " + FieldName + " FROM " + TableName + " b WHERE ISNUMERIC(b." +
              FieldName + ") AND b." + FieldName + "=a.CardID)";
            return ret;
        }
        ...
        public string GetSQL(DBCode code, string[] Param)
        {
            ...
            switch (code)
            {
                case DBCode.DB_000001:
                    ...
                    switch (I)
                    {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            ret = "UPDATE SY_Oprt SET OprtLastLoginTime=now() WHERE OprtNo='" + Param[1] + "'";
                            break;
                        case 4:
                        ...
                        case 9:
                            ret = "INSERT INTO SY_Log(OprtTime,OprtModule,OprtTool,OprtDetail,OprtNoName,OprtComputerName) " +
                              "VALUES(now(),'" + Param[1] + "','" + Param[2] + "','" + Param[3] + "','" + Param[4] + "','" +
                              Param[5] + "')";
                            break;
                        case 10:
                        ...
                        case 18:
                            ...
                            break;
                        case 100:
                            ret = "DELETE FROM KQ_EmpShift WHERE EmpShiftDate>=CDate('" + Param[1] +
                              "') AND EmpShiftDate<=CDate('" + Param[2] + "')";
                            break;
                        case 101:
                            ret = "DELETE FROM KQ_DepShift WHERE DepShiftDate>=CDate('" + Param[1] +
                              "') AND DepShiftDate<=CDate('" + Param[2] + "')";
                            break;
                        case 102:
                            ret = "DELETE FROM KQ_Holiday WHERE HolidayBeginTime>=CDate('" + Param[1] +
                              "') AND HolidayEndTime<=CDate('" + Param[2] + "')";
                            break;
                        case 103:
                            ret = "DELETE FROM KQ_EmpDayOff WHERE BeginTime>=CDate('" + Param[1] +
                              "') AND EndTime<=CDate('" + Param[2] + "')";
                            break;
                        case 104:
                            ret = "DELETE FROM KQ_EmpOtSure WHERE BeginTime>=CDate('" + Param[1] +
                              "') AND EndTime<=CDate('" + Param[2] + "')";
                            break;
                        case 105:
                            ret = "DELETE FROM KQ_KQData WHERE IsSignIn AND KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 106:
                            ret = "DELETE FROM KQ_KQDataPhoto WHERE [GUID] IN(SELECT [GUID] FROM KQ_KQData WHERE KQDate>=CDate('" +
                              Param[1] + "') AND KQDate<=CDate('" + Param[2] + "'))\r\n";
                            ret += "DELETE FROM KQ_KQData WHERE KQDate>=CDate('" + Param[1] + "') AND KQDate<=CDate('" +
                              Param[2] + "')\r\n";
                            break;
                        case 107:
                            ret = "DELETE FROM KQ_KQDataFilter WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')\r\n";
                            ret += "DELETE FROM KQ_KQDataFilterMark WHERE KQDate>=CDate('" + Param[1] + "') AND KQDate<=CDate('" +
                             Param[2] + "')";
                            break;
                        case 108:
                            ret = "DELETE FROM KQ_KQReportDay WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 109:
                        ...
                        case 111:
                            ...
                            break;
                        case 112:
                            ret = "DELETE FROM KQ_MJData WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 113:
                            ret = "DELETE FROM MJ_OpenData WHERE MJDateTime>=CDate('" + Param[1] +
                               "') AND MJDateTime<=CDate('" + Param[2] + "')";
                            break;
                        case 114:
                            ret = "DELETE FROM MJ_AlarmData WHERE AlarmTime>=CDate('" + Param[1] +
                              "') AND AlarmTime<=CDate('" + Param[2] + "')";
                            break;
                        case 115:
                            ret = "DELETE FROM MJ_SeaPersonIDCard WHERE KQDateTime>=CDate('" + Param[1] +
                              "') AND KQDateTime<=CDate('" + Param[2] + "')";
                            break;
                        case 200:
                        ...
                        case 303:
                            ...
                            break;
                        case 500:
                            ret = "SELECT TOP 9000 * FROM KQ_EmpShift WHERE EmpShiftDate>=CDate('" + Param[1] +
                              "') AND EmpShiftDate<=CDate('" + Param[2] + "')";
                            break;
                        case 501:
                            ret = "SELECT TOP 9000 * FROM KQ_DepShift WHERE DepShiftDate>=CDate('" + Param[1] +
                              "') AND DepShiftDate<=CDate('" + Param[2] + "')";
                            break;
                        case 502:
                            ret = "SELECT TOP 9000 * FROM KQ_Holiday WHERE HolidayBeginTime>=CDate('" + Param[1] +
                              "') AND HolidayEndTime<=CDate('" + Param[2] + "')";
                            break;
                        case 503:
                            ret = "SELECT TOP 9000 * FROM KQ_EmpDayOff WHERE BeginTime>=CDate('" + Param[1] +
                              "') AND EndTime<=CDate('" + Param[2] + "')";
                            break;
                        case 504:
                            ret = "SELECT TOP 9000 * FROM KQ_EmpOtSure WHERE BeginTime>=CDate('" + Param[1] +
                              "') AND EndTime<=CDate('" + Param[2] + "')";
                            break;
                        case 505:
                            ret = "SELECT TOP 9000 * FROM KQ_KQData WHERE IsSignIn=1 AND KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 506:
                            ret = "SELECT TOP 9000 * FROM KQ_KQData WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 507:
                            ret = "SELECT TOP 9000 * FROM KQ_KQDataFilter WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 508:
                            ret = "SELECT TOP 9000 * FROM KQ_KQReportDay WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 509:
                        ...
                        case 511:
                            ...
                            break;
                        case 512:
                            ret = "SELECT TOP 9000 * FROM KQ_MJData WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 513:
                            ret = "SELECT TOP 9000 * FROM MJ_OpenData WHERE MJDateTime>=CDate('" + Param[1] +
                               "') AND MJDateTime<=CDate('" + Param[2] + "')";
                            break;
                        case 514:
                            ret = "SELECT TOP 9000 * FROM MJ_AlarmData WHERE AlarmTime>=CDate('" + Param[1] +
                              "') AND AlarmTime<=CDate('" + Param[2] + "')";
                            break;
                        case 515:
                            ret = "SELECT TOP 9000 * FROM MJ_SeaPersonIDCard WHERE KQDateTime>=CDate('" + Param[1] +
                             "') AND KQDateTime<=CDate('" + Param[2] + "')";
                            break;
                        case 550:
                            ...
                            break;
                        case 600:
                            ret = "DELETE FROM KQ_EmpShift WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_EmpShift WHERE EmpShiftDate>=CDate('" + Param[1] +
                              "') AND EmpShiftDate<=CDate('" + Param[2] + "')))";
                            break;
                        case 601:
                            ret = "DELETE FROM KQ_DepShift WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_DepShift WHERE DepShiftDate>=CDate('" + Param[1] +
                              "') AND DepShiftDate<=CDate('" + Param[2] + "')))";
                            break;
                        case 602:
                            ret = "DELETE FROM KQ_Holiday WHERE (HolidayID IN(SELECT TOP 9000 HolidayID FROM KQ_Holiday WHERE HolidayBeginTime>=CDate('" + Param[1] +
                              "') AND HolidayEndTime<=CDate('" + Param[2] + "')))";
                            break;
                        case 603:
                            ret = "DELETE FROM KQ_EmpDayOff WHERE (EmpDayOffID IN(SELECT TOP 9000 EmpDayOffID FROM KQ_EmpDayOff WHERE BeginTime>=CDate('" + Param[1] +
                              "') AND EndTime<=CDate('" + Param[2] + "')))";
                            break;
                        case 604:
                            ret = "DELETE FROM KQ_EmpOtSure WHERE (EmpOtSureID IN(SELECT TOP 9000 EmpOtSureID FROM KQ_EmpOtSure WHERE BeginTime>=CDate('" + Param[1] +
                              "') AND EndTime<=CDate('" + Param[2] + "')))";
                            break;
                        case 605:
                            ret = "DELETE FROM KQ_KQData WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_KQData WHERE IsSignIn=1 AND KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')))";
                            break;
                        case 606:
                            ret = "DELETE FROM KQ_KQDataPhoto WHERE [GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_KQData WHERE KQDate>=CDate('" + Param[1] + "') AND KQDate<=CDate('" + Param[2] + "'))\r\n";
                            ret += " DELETE FROM KQ_KQData WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_KQData WHERE KQDate>=CDate('" + Param[1] +"') AND KQDate<=CDate('" + Param[2] + "')))";
                            break;
                        case 607:
                            ret = "DELETE FROM KQ_KQDataFilter WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_KQDataFilter WHERE KQDate>=CDate('" + Param[1] +"') AND KQDate<=CDate('" + Param[2] + "')))\r\n";
                            ret += " DELETE FROM KQ_KQDataFilterMark WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_KQDataFilterMark WHERE KQDate>=CDate('" + Param[1] +"') AND KQDate<=CDate('" + Param[2] + "')))";
                            break;
                        case 608:
                            ret = "DELETE FROM KQ_KQReportDay WHERE (EmpNo IN(SELECT TOP 9000 EmpNo FROM KQ_KQReportDay WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "'))) AND KQDate>=CDate('" + Param[1] + "') AND KQDate<=CDate('" + Param[2] + "')";
                            break;
                        case 609:
                        ...
                        case 611:
                            ...
                            break;
                        case 612:
                            ret = "DELETE FROM KQ_MJDataPhoto WHERE [GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_MJData WHERE KQDate>=CDate('" + Param[1] + "') AND KQDate<=CDate('" + Param[2] + "'))\r\n";
                            ret += " DELETE FROM KQ_MJData WHERE [GUID] IN(SELECT TOP 9000 [GUID] FROM KQ_MJData WHERE KQDate>=CDate('" + Param[1] + "') AND KQDate<=CDate('" + Param[2] + "'))";
                            break;
                        case 613:
                            ret = "DELETE FROM MJ_OpenData WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM MJ_OpenData WHERE MJDateTime>=CDate('" + Param[1] +
                              "') AND MJDateTime<=CDate('" + Param[2] + "')))";
                            break;
                        case 614:
                            ret = "DELETE FROM MJ_AlarmData WHERE ([GUID] IN(SELECT TOP 9000 [GUID] FROM MJ_AlarmData WHERE AlarmTime>=CDate('" + Param[1] +
                              "') AND AlarmTime<=CDate('" + Param[2] + "')))";
                            break;
                        case 615:
                            ret = "DELETE FROM MJ_SeaPersonIDCard WHERE (GUID IN(SELECT TOP 9000 GUID FROM MJ_SeaPersonIDCard WHERE KQDateTime>=CDate('" + Param[1] + "') AND KQDateTime<=CDate('" + Param[2] + "')))\r\n";
                            ret = "DELETE FROM MJ_SeaPersonIDCard WHERE (GUID IN(SELECT TOP 9000 GUID FROM MJ_SeaPersonIDCard WHERE KQDateTime>=CDate('" + Param[1] +
                              "') AND KQDateTime<=CDate('" + Param[2] + "')))";
                            break;
                        case 650:
                    ...
                    switch (I)
                    {
                        ...
                        case 202:
                            ret = "INSERT INTO SY_MacParam([GUID],MacSN, DoorMagneticType, Antiback," +
                                   ...
                                   "WiegandType,ServerIPAddress,ServerPort,WiegandOutput,WiegandInput) VALUES('" + GetGUID().ToString() + "','" + Param[1] + "','" + Param[2] + "','" + Param[3] + "','" +
                ...
                case DBCode.DB_000100:
                    ...
                    {
                        case 0:
                            ret = "SELECT IIF(ISNULL(MAX(DepartID)),0,MAX(DepartID)) AS MaxID FROM RS_Depart " +
                              "WHERE DepartUpID='" + Param[1] + "' AND ISNUMERIC(DepartID)";
                            break;
                        case 1:
                ...
                case DBCode.DB_000101:
                    ...
                    {
                        case 0:
                            ret = "SELECT *,IIF(FingerPrivilege=1,'" + Param[2] + "','" + Param[1] + "') AS FingerPrivilegeName " +
                              "FROM VRS_Emp WHERE NOT IsDimission";
                            break;
                        case 1:
                            ret = " ORDER BY EmpNo";
                            break;
                        ...
                        case 7:
                            ...
                            break;
                        case 8:
                            ret = "DELETE FROM RS_EmpFingerInfo WHERE FingerNo=(SELECT FingerNo FROM RS_Emp WHERE EmpNo='" +
                              Param[1] + "')";
                            break;
                        case 9:
                            ret = "DELETE FROM RS_Emp WHERE EmpNo='" + Param[1] + "'";
                            break;
                        case 10:
                            ret = "SELECT EmpNo FROM RS_Emp WHERE FingerNo=" + Param[1];
                            break;
                        case 11:
                            ret = "SELECT IIF(ISNULL(MAX(FingerNo)),0,MAX(FingerNo))+1 AS MaxID FROM RS_Emp";
                            break;
                        case 12:
                        ...
                        case 22:
                            ...
                            break;
                        case 23:
                            ret = GetMaxIDSQL("FingerNo", "RS_Emp");
                            break;
                        case 24:
                        ...
                        case 32:
                            ...
                            break;
                        case 33:
                            ret = "DELETE FROM RS_EmpDynamicInfo WHERE EmpNo='" + Param[1] + "'";
                            break;
                        case 34:
                        ...
                        case 101:
                            ret = "SELECT EmpSex,COUNT(1) AS RecCount " +
                              "FROM VRS_Emp WHERE NOT IsDimission GROUP BY EmpSex";
                            break;
                        case 102:
                            ret = "SELECT DepartName AS DepartName,COUNT(1) AS RecCount " +
                              "FROM VRS_Emp WHERE NOT IsDimission GROUP BY DepartName";
                            break;
                        case 201:
                ...
                case DBCode.DB_000102:
                    ...
                    switch (I)
                    {
                        case 0:
                            ret = "SELECT *,IIF(FingerPrivilege=1,'" + Param[2] + "','" + Param[1] + "') AS FingerPrivilegeName " +
                              "FROM VRS_EmpDimission WHERE 1=1";
                            break;
                        case 1:
                            ret = " ORDER BY EmpNo";
                            break;
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            ret = "UPDATE RS_Emp SET DimissionOprt='" + Param[1] + "',DimissionDate=CDate('" + Param[2] +
                ...
                case DBCode.DB_000206:
                    ...
                    switch (I)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            ret = "INSERT INTO KQ_EmpShift([GUID],EmpNo,EmpShiftDate,ShiftNo) VALUES('" + GetGUID().ToString() +
                              "','" + Param[1] + "',CDate('" + Param[2] + "'),'" + Param[3] + "')";
                            break;
                        case 3:
                            ret = "DELETE FROM KQ_EmpShift WHERE EmpNo='" + Param[1] + "' AND EmpShiftDate>=CDate('" +
                              Param[2] + "') AND EmpShiftDate<=CDate('" + Param[3] + "')";
                            break;
                    }
                    break;
                case DBCode.DB_000207:
                    ...
                    switch (I)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            ret = "INSERT INTO KQ_DepShift([GUID],DepartID,DepShiftDate,ShiftNo) VALUES('" + GetGUID().ToString() +
                              "','" + Param[1] + "',CDate('" + Param[2] + "'),'" + Param[3] + "')";
                            break;
                        case 3:
                            ret = "DELETE FROM KQ_DepShift WHERE DepartID='" + Param[1] + "' AND DepShiftDate>=CDate('" +
                              Param[2] + "') AND DepShiftDate<=CDate('" + Param[3] + "')";
                            break;
                    }
                    break;
                case DBCode.DB_000208:
                    ...
                    switch (I)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            ret = "SELECT * FROM VKQ_EmpShiftsTotal WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                ...
                case DBCode.DB_000209:
                    ...
                    switch (I)
                    {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            ret = "INSERT INTO KQ_Holiday(HolidayID,HolidayName,HolidayBeginTime,HolidayEndTime) VALUES('" +
                              GetGUID().ToString() + "','" + Param[1] + "',CDate('" + Param[2] + "'),CDate('" + Param[3] + "'))";
                            break;
                        case 4:
                            ret = "UPDATE KQ_Holiday SET HolidayName='" + Param[1] + "',HolidayBeginTime=CDate('" + Param[2] +
                              "'),HolidayEndTime=CDate('" + Param[3] + "') WHERE HolidayID='" + Param[4] + "'";
                            break;
                    }
                    break;
                    ...
                    switch (I)
                    {
                        ...
                        case 4:
                            ret = "INSERT INTO KQ_EmpDayOff(EmpDayOffID,EmpNo,SortID,BeginTime,EndTime,DayOffReason,OprtNo," +
                              "OprtDate) VALUES('" + GetGUID().ToString() + "','" + Param[1] + "','" + Param[2] + "',CDate('" +
                              Param[3] + "'),CDate('" + Param[4] + "'),'" + Param[5] + "','" + Param[6] + "',now())";
                            break;
                        case 5:
                            ret = "UPDATE KQ_EmpDayOff SET SortID='" + Param[1] + "',BeginTime=CDate('" + Param[2] +
                              "'),EndTime=CDate('" + Param[3] + "'),DayOffReason='" + Param[4] + "',OprtNo='" + Param[5] +
                              "',OprtDate=now() WHERE EmpDayOffID='" + Param[6] + "'";
                            break;
                        case 6:
                        ...
                        case 9:
                            ...
                            break;
                        case 10:
                            ret = "SELECT * FROM VKQ_EmpDayOff WHERE EmpNo='" + Param[1] + "' AND BeginTime=CDate('" + Param[2] + "') " +
                                "AND EndTime=CDate('" + Param[3] + "')";
                            break;
                    }
                    break;
                    ...
                    switch (I)
                    {
                        ...
                        case 4:
                            ret = "INSERT INTO KQ_EmpOtSure(EmpOtSureID,EmpNo,SortID,BeginTime,EndTime,AheadHrs,AheadMins," +
                              "DeferHrs,DeferMins,OtReason,OprtNo,OprtDate) VALUES('" + GetGUID().ToString() + "','" +
                              Param[1] + "','" + Param[2] + "',CDate('" + Param[3] + "'),CDate('" + Param[4] + "')," +
                              Param[5] + "," + Param[6] + "," + Param[7] + "," + Param[8] + ",'" + Param[9] + "','" +
                              Param[10] + "',now())";
                            break;
                        case 5:
                            ret = "UPDATE KQ_EmpOtSure SET SortID='" + Param[1] + "',BeginTime=CDate('" + Param[2] +
                              "'),EndTime=CDate('" + Param[3] + "'),AheadHrs=" + Param[4] + ",AheadMins=" + Param[5] +
                              ",DeferHrs=" + Param[6] + ",DeferMins=" + Param[7] + ",OtReason='" + Param[8] + "',OprtNo='" +
                              Param[9] + "',OprtDate=now() WHERE EmpOtSureID='" + Param[10] + "'";
                            break;
                        case 6:
                        ...
                        case 9:
                            ...
                            break;
                        case 10:
                            ret = "SELECT * FROM VKQ_EmpOtSure WHERE EmpNo='" + Param[1] + "' AND BeginTime=CDate('" + Param[2] + "') " +
                                "AND EndTime=CDate('" + Param[3] + "')";
                            break;
                    }
                    break;
                case DBCode.DB_000212:
                    ...
                    {
                        case 0:
                            ret = "SELECT * FROM VKQ_KQData WHERE IsSignIn ORDER BY EmpNo,KQDate,KQTime";
                            break;
                        case 1:
                        ...
                        case 4:
                            ret = "UPDATE KQ_KQData SET KQDate='" + Param[1] + "',KQTime=" + Param[2] + ",Remark='" +
                              Param[3] + "',OprtNo='" + Param[4] + "',OprtDate=now() WHERE [GUID]='" + Param[5] + "'";
                            break;
                        case 5:
                            ret = "SELECT * FROM VKQ_KQData WHERE IsSignIn";
                            break;
                        case 6:
                ...
                case DBCode.DB_000214:
                    ...
                    {
                        case 0:
                            ret = "SELECT * FROM VKQ_KQData WHERE KQDate>=CDate('" + Param[6] +
                              "') AND KQDate<=CDate('" + Param[7] + "')";
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            ret = @"SELECT * FROM VKQ_MJData WHERE KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            ret = @"SELECT * FROM VKQ_MJData WHERE (IsAlarm=0 OR IsAlarm=null) AND KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            ret += GetEmpInfoSQL(Param[3], Param[4]) + GetMacSNInfoSQL(Param[5], Param[6]) + GetInOutModeInfoSQL(Param[7]);
                            ret += " ORDER BY KQDate,KQTime";
                            break;
                        case 6:
                            ret = @"SELECT * FROM VKQ_MJData WHERE IsAlarm<>0 AND KQDate>=CDate('" + Param[1] +
                              "') AND KQDate<=CDate('" + Param[2] + "')";
                            ret += GetEmpInfoSQL(Param[3], Param[4]) + GetMacSNInfoSQL(Param[5], Param[6]) + GetInOutModeInfoSQL(Param[7]);
                            ret += " ORDER BY KQDate,KQTime";
                            break;
                        case 7:
                            ret = "SELECT * FROM SY_Log WHERE OprtTime>=CDate('" + Param[1] + "') AND OprtTime<=CDate('" + Param[2] + "')";
                            ret += GetMacSNSQL(Param[3]) + GetOprtModuleSQL(Param[4]) + GetOprtToolSQL(Param[5]);
                            ret += " ORDER BY OprtTime desc";
                            break;
                        case 8:
                            ret = "SELECT VerifyModeName FROM VKQ_KQData ";
                            break;
                        case 10:
                            ret = "SELECT * FROM VDI_SeaSnapShots WHERE OprtDate>=CDate('" + Param[1] +
                              "') AND OprtDate<=CDate('" + Param[2] + "') order by OprtDate desc";
                            break;
                        case 11:
                        ...
                        case 12:
                            ...
                            break;
                        case 13:
                            ret = "SELECT * FROM VKQ_KQData WHERE KQDateTime>=CDate('" + Param[1] +
                              "') AND KQDateTime<=CDate('" + Param[2] + "')";
                            ret += " ORDER BY DepartID,EmpNo,KQDate,KQTime";
                            break;
                        case 14:
                        ...
                        case 15:
                            ...
                            break;
                        case 20:
                            ret = @"SELECT * FROM VMJ_SeaPersonIDCard WHERE KQDateTime>=CDate('" + Param[1] +
                              "') AND KQDateTime<=CDate('" + Param[2] + "')";
                ...
                case DBCode.DB_000215:
                    ...
                    {
                        case 0:
                            ret = "SELECT * FROM VKQ_KQDataFilterMark WHERE KQDate>=CDate('" + Param[6] +
                              "') AND KQDate<=CDate('" + Param[7] + "')";
                            ret += GetEmpInfoSQL(Param[1], Param[2]) + GetDepartInfoSQL(Param[3], Param[4], Param[5]);
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            ret = "SELECT KQDateTime FROM KQ_KQData WHERE EmpNo='" + Param[1] + "' AND KQDate=CDate('" +
                              Param[2] + "') ORDER BY KQDate,KQTime";
                            break;
                        case 3:
                ...
                case DBCode.DB_000216:
                    ...
                    {
                        case 0:
                            ret = "SELECT * FROM VKQ_KQReportDay WHERE KQDate>=CDate('" + Param[6] +
                              "') AND KQDate<=CDate('" + Param[7] + "')";
                            ret += GetEmpInfoSQL(Param[1], Param[2]) + GetDepartInfoSQL(Param[3], Param[4], Param[5]);
                            ...
                            break;
                        case 1:
                            ret = "SELECT * FROM VKQ_KQData WHERE EmpNo='" + Param[1] + "' AND KQDate=CDate('" +
                ...
                case DBCode.DB_000300:
                    ...
                    switch (I)
                    {
                        ...
                        case 102:
                            ...
                            break;
                        case 103:
                            // ret = GetMaxIDSQL("MacSN", "DI_MacInfo");
                            ret = "SELECT IIF(ISNULL(MIN(CardID)),(SELECT top 1 MacSN+1 FROM DI_MacInfo " +
                                               "where IsNumeric(MacSN) order by MacSN+1 desc) ,MIN(CardID)) " +
                                               "AS CardIDX FROM (SELECT(SELECT COUNT(1)+1 FROM DI_MacInfo a " +
                                               "WHERE ISNUMERIC(a.MacSN) AND a.MacSN>b.MacSN) AS CardID FROM " +
                                               "DI_MacInfo b) a WHERE NOT EXISTS(SELECT MacSN FROM DI_MacInfo b " +
                                               "WHERE ISNUMERIC(b.MacSN) AND b.MacSN=a.CardID)";
                            break;
                        case 104:
                        ...
                        case 106:
                            ...
                            break;
                        case 107:
                            ret = "SELECT * FROM VRS_Emp WHERE ((LEN(CardNo10)=10 AND ISNUMERIC(CardNo10)) OR " +
                        ...
                        case 108:
                            ...
                            break;
                        case 109:
                            ret = " AND ((LEN(CardNo10)=10 AND ISNUMERIC(CardNo10)) OR " +
                        ...
                        case 113:
                            ret = "INSERT INTO KQ_SuperData(GUID,DevID,SEnrollNo,GEnrollNo,ManID,ManName,BakNo,BakName,STime,OprtNo," +
                             ...
                             Param[5] + "'," + Param[6] + ",'" + Param[7] + "','" + Param[8] + "','" + Param[9] + "',now())";
                            break;
                        case 114:
                        ...
                        case 401:
                            ...
                            break;
                        case 402:
                            ret = GetMaxIDSQL("PassTimeID", "DI_PsssTime");
                            break;
                        case 403:
                        ...
                        case 405:
                            ret = "UPDATE DI_PsssTime SET PassTimeName='" + Param[2] + "',T1S='" + Param[3] + "',T1E='" +
                              ...
                              "',OprtDate=now() WHERE PassTimeID=" + Param[1];
                            break;
                        case 500:
                        ...
                        case 504:
                            ret = "INSERT INTO DI_Power([GUID],MacSN,EmpNo,SunID,MonID,TueID,WedID,ThuID,FriID,SatID,OprtNo," +
                               "OprtDate,StartDate,EndDate) VALUES('" + GetGUID().ToString() + "','" + Param[1] + "','" + Param[2] + "'," +
                               Param[3] + "," + Param[4] + "," + Param[5] + "," + Param[6] + "," + Param[7] + "," + Param[8] + "," +
                               Param[9] + ",'" + Param[10] + "',now()," + Param[11] + "," + Param[12] + ")";
                            break;
                        case 505:
                            ret = "UPDATE DI_Power SET SunID=" + Param[3] + ",MonID=" + Param[4] + ",TueID=" + Param[5] + ",WedID=" +
                              ...
                              Param[10] + "',OprtDate=now(),StartDate=" + Param[11] + ",EndDate=" + Param[12] + " WHERE MacSN='" +
                              Param[1] + "' AND EmpNo='" + Param[2] + "'";
                            break;
                        case 506:
                            ret = "DELETE FROM DI_Power WHERE EmpNo='" + Param[1] + "'";
                            break;
                        case 507:
                        ...
                        case 514:
                            ret = "INSERT INTO DI_SeaPower([GUID],MacSN,EmpNo,OprtNo," +
                               "OprtDate,StartDate,EndDate) VALUES('" + GetGUID().ToString() + "','" + Param[1] + "','" + Param[2] + "','" +
                               Param[3] + "',now()," + Param[4] + "," + Param[5] + ")";
                            break;
                        case 515:
                            ret = "UPDATE DI_SeaPower SET OprtNo='" +
                              Param[3] + "',OprtDate=now(),StartDate=" + Param[4] + ",EndDate=" + Param[5] + " WHERE MacSN='" +
                              Param[1] + "' AND EmpNo='" + Param[2] + "'";
                            break;
                        case 516:
                            ret = "DELETE FROM DI_SeaPower WHERE EmpNo='" + Param[1] + "'";
                            break;
                        case 517:
                        ...
                        case 555:
                            ...
                            break;
                        case 556:
                            ret = "DELETE FROM DI_StarPower WHERE EmpNo='" + Param[1] + "'";
                            break;
                        case 557:
                        ...
                        case 602:
                            ret = "UPDATE DI_Power SET SunID=" + Param[2] + ",MonID=" + Param[3] + ",TueID=" + Param[4] + ",WedID=" +
                              ...
                              "',OprtDate=now(),StartDate=" + Param[10] + ",EndDate=" + Param[11] + " WHERE [GUID]='" + Param[1] + "'";
                            break;
                        case 603:
                        ...
                        case 612:
                            ret = "UPDATE DI_SeaPower SET OprtNo='" + Param[2] +
                              "',OprtDate=now(),StartDate=" + Param[3] + ",EndDate=" + Param[4] + " WHERE [GUID]='" + Param[1] + "'";
                            break;
                        case 613:
                        ...
                        case 623:
                            ...
                            break;
                        case 700:
                            ret = "INSERT INTO MJ_TemporaryData([GUID],MJDateTime,MacSN,EmpNo,EmpName) VALUES('" + GetGUID().ToString() + "',CDate('" + Param[1] + "')," +
                                            "'" + Param[2] + "','" + Param[3] + "','" + Param[4] + "')";
                            break;
                        case 701:
                        ...
                        case 704:
                            ret = "INSERT INTO MJ_OpenData([GUID],MJDateTime,MacSN,MacDesc,InOutModeName,EmpNoOne,EmpNoTwo," +
                                "EmpNoTree,EmpNoFour,EmpNoFive) VALUES('" + GetGUID().ToString() + "',CDate('" + Param[1] + "')," +
                                 "'" + Param[2] + "','" + Param[3] + "','" + Param[4] + "'," +
                                 "'" + Param[5] + "','" + Param[6] + "','" + Param[7] + "','" + Param[8] + "','" + Param[9] + "')";
                            break;
                        case 705:
                            ret = "SELECT [GUID] FROM MJ_OpenData WHERE MJDateTime=CDate('" + Param[1] + "') AND EmpNoOne='" + Param[2] + "'";
                            break;
                        case 706:
                            ret = "UPDATE MJ_OpenData SET MJDateTime=CDate('" + Param[2] + "'),MacSN='" + Param[3] + "',MacDesc='" + Param[4] + "',InOutModeName='" +
                              Param[5] + "',EmpNoOne='" + Param[6] + "',EmpNoTwo='" + Param[7] + "',EmpNoTree='" + Param[8] + "' WHERE [GUID]='" + Param[1] + "'";
                            break;
                        case 707:
                            ret = "SELECT * FROM VMJ_OpenData WHERE MJDateTime>=CDate('" + Param[1] +
                            "') AND MJDateTime<=CDate('" + Param[2] + "')";
                            ret += GetMJEmpInfoSQL(Param[3]) + GetMacSNInfoSQL(Param[4], Param[5]) + GetInOutModeInfoSQL(Param[6]); ;
                            ret += " ORDER BY MJDateTime";
                            break;
                        case 708:
                            ret = "INSERT INTO MJ_TemporaryData([GUID],MJDateTime,MacSN,EmpNo,EmpName) VALUES('" + Param[1] + "',CDate('" + Param[2] + "')," +
                        ...
                        case 710:
                            ...
                            break;
                        case 800:
                            ret = "INSERT INTO MJ_AlarmData([GUID],AlarmTime,MacSN,MacDesc,AlarmMode) VALUES('" + GetGUID().ToString() + "',CDate('" + Param[1] + "')," +
                                 "'" + Param[2] + "','" + Param[3] + "','" + Param[4] + "')";
                            break;
                        case 801:
                            ret = "SELECT [GUID] FROM VMJ_AlarmData WHERE AlarmTime=CDate('" + Param[1] + "') AND MacSN='" + Param[2] + "'";
                            break;
                        case 802:
                            ret = "UPDATE MJ_AlarmData SET AlarmTime=CDate('" + Param[2] + "'),MacSN='" + Param[3] + "',MacDesc='" + Param[4] + "',AlarmMode='" +
                        ...
                        case 803:
                            ...
                            break;
                        case 804:
                            ret = "SELECT * FROM VMJ_AlarmData WHERE AlarmTime>=CDate('" + Param[1] +
                            "') AND AlarmTime<=CDate('" + Param[2] + "')";
                ...
                case DBCode.DB_000400:
                    ...
                    switch (I)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            ret = "SELECT *,IIF(RuleMode=1,'{0}','{1}') AS Mode FROM VGZ_Rule";
                            break;
                        case 2:
